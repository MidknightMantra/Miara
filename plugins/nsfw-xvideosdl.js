...
const handler = async (m, { conn, args, command, usedPrefix, text }) => {
  ...
  const translator = _translate.plugins.adult_xvideosdl

  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw `${translator.texto1} #enable modohorny*`;
  if (!args[0]) throw `${translator.texto2} ${usedPrefix + command} https://www.xvideos.com/video70389849/pequena_zorra_follada_duro*`;
  try {
