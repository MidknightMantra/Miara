...
const handler = async (m, {conn, text, participants, isAdmin, isOwner, usedPrefix, command}) => {
  ...
  const translator = _translate.plugins.owner_bcgc2
  ...
  if (!m.quoted) throw `${translator.texto1[0]} *${usedPrefix + command}* ${translator.texto1[1]}`;
  ...
  m.reply(`${translator.texto2[0]} ${groups.length} ${translator.texto2[1]}`);
};
handler.help = ['bcgc2'];
handler.tags = ['owner'];
