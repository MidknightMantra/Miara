...
const handler = async (m, {text, conn, usedPrefix, command}) => {
  ...
  const translator = _translate.plugins.owner_block_unblock

  const why = `${translator.texto1} ${usedPrefix + command} @${m.sender.split('@')[0]}*`;
  ...
  if (res[0]) conn.reply(m.chat, `${translator.texto2[0]} ${command} ${translator.texto2[1]} ${res ? `${res.map((v) => '@' + v.split('@')[0])}` : ''}*`, m, {mentions: res});
};
