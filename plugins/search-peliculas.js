...
const handler = async (m, {text, usedPrefix, command, conn}) => {
 try {
  ...
  const translator = _translate.plugins.buscador_peliculas
  ...
  if (aaaa == '') throw `*${translator.texto2}*`;                                        /* https://wwv.cuevana8.com */
  const res = await aaaa.map((v) => `*${translator.texto3[0]}* ${v.title}\n*${translator.texto3[1]}* ${v.link}`).join`\n\n─────────────────\n\n`;
  const ads = `*${translator.texto3[2]}* ${translator.texto3[3]}\n*${translator.texto3[4]}* https://block-this.com/block-this-latest.apk\n\n≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣\n\n`;
