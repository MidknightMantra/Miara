...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  ...
  const translator = _translate.plugins.sticker_sticker
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      else return m.reply(`${translator.texto2} ${usedPrefix}s https://telegra.ph/file/0dc687c61410765e98de2.jpg*`);
    }
  } catch (e) {
    console.error(e);
    if (!stiker) stiker = e;
   ...
   else return m.reply(`${translator.texto3}` + ` ${usedPrefix + command}*`);
  }
};
handler.help = ['sfull'];
