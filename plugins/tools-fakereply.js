...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  const translator = _translate.plugins.herramientas_fakereply

  if (!text) return m.reply(`${translator.texto1[0]}\n\n*${usedPrefix + command}* ${translator.texto1[1]} @${m.sender.split`@`[0]} a`, null, {mentions: [m.sender]});
  const cm = copy(m);
  let who;
  if (text.includes('@0')) who = '0@s.whatsapp.net';
  else if (m.isGroup) who = cm.participant = await await m.mentionedJid[0];
  else who = m.chat;
  if (!who) return m.reply(`${translator.texto1[0]}\n\n*${usedPrefix + command}* ${translator.texto1[1]} @${m.sender.split`@`[0]} a`, null, {mentions: [m.sender]});
  cm.key.fromMe = false;
